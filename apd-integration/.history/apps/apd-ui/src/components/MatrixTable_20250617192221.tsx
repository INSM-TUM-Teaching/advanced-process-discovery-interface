...
const getInterpretation = (dependency: Dependency | undefined): any => {
    ...
    if (temporal_dependency.type == "Eventual") {
        temporal_explanation = `There is an eventual temporal dependency. Thus, ${temporal_dependency.direction == "Forward" ? <strong>{dependency.to.toUpperCase()}</strong> : dependency.from.toUpperCase()
