...
const RunProject: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchClassificationFromMatrixResult = async () => {
      fetchClassificationFromMatrix(new URL(project.endpoints[0].structure), project.matrix)
      setClassificationResults(results);
    }

    if (project.matrix) {
      setMatrixResults([project.matrix]);
      fetchClassificationFromMatrixResult();
      return;
    }
