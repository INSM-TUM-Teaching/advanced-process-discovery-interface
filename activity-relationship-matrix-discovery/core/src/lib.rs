...
pub use logic::process_xes_content;
...
pub struct DependencyMatrix {
    pub matrix: Vec<Vec<DependencyCell>>,
...
pub fn generate_adj_matrix_from_activities_and_traces(
    ...
    let mut matrix: Vec<Vec<DependencyCell>> = vec![];
    ...
    for from in &activities_sorted {
        ...
        for to in &activities_sorted {
            if from != to {
                let temporal = check_temporal_dependency(from, to, &converted_traces, temporal_threshold);
                let existential = check_existential_dependency(from, to, &converted_traces, existential_threshold);
                ...
                metrics.update(&temporal, &existential);

                row.push(DependencyCell {
